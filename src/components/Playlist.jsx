...
export default function Playlist() {
  ...
  function renderPlayer() {
      ...
      return vid ? (
        <div className="playlist-player playlist-player-video">
          ...
          <div className="video-wrapper">
            <iframe
              src={`https://www.youtube-nocookie.com/embed/${vid}?modestbranding=1&rel=0&iv_load_policy=3`}
    ...
    if (currentType === 'video') {
      return (
        <div className="playlist-player playlist-player-video">
          <span className="player-title">{currentSong.title}</span>
          <div className="video-wrapper">
            <video ref={audioRef} src={currentSong.url} controls autoPlay playsInline onEnded={() => setCurrentIdx(-1)} />
          </div>
        </div>
      )
    }
    ...
    return (
      <div className="playlist-player">
        <span className="player-title">{currentSong.title}</span>
        <audio ref={audioRef} key={currentSong.id} src={currentSong.url} controls autoPlay onEnded={() => setCurrentIdx(-1)} style={{ width: '100%' }} />
      </div>
    )
  }
